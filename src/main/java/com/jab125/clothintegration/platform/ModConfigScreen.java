...
public class ModConfigScreen {
	public static Screen getConfigScreen(Screen prev) {
		...
		client.addEntry(configBuilder.entryBuilder().startBooleanToggle(Text.literal("Configured Backgrounds"), config.configuredBackgrounds).setDefaultValue(defaultConfig.configuredBackgrounds).setTooltip(Text.literal("For 1.20.4 and below.")).setSaveConsumer(a -> config.configuredBackgrounds = a).build());
